...
DRReturn GlyphGrid::addToGrid(u32 bezierIndex, DRVector2* vectors, u16 vectorCount)
{
	...
	DRBoundingBox bb(vectors, vectorCount);
...
GlyphCalculate::GlyphCalculate()
	: mGlyphGrid(GLYPHE_GRID_SIZE), mBezierIndices(NULL), mBezierNodeMax(0)
{
...
void GlyphCalculate::addPointToBezier(DRVector2i p, int conturIndex, bool onCurve /*= true*/)
{
	...
	if (onCurve && mTempPoints.size() > 1) {
		...
		mBezierNodeMax = max(mBezierNodeMax, (u8)mTempPoints.size());
		int index = 0;
...
DRReturn GlyphCalculate::loadGlyphInfos(FT_ULong c, FT_Face face)
{
	...
	mGlypInfos = face->glyph;
	//FT_BBox boundingBox = face->bbox;
	mGlyphMetrics = mGlypInfos->metrics;

	if (mGlypInfos->format != FT_GLYPH_FORMAT_OUTLINE) {
...
void GlyphCalculate::writeDebugInfos(FT_ULong c, FT_Face face)
{
	...
	for (u32 i = 0; i < strlen(face->family_name); i++) {
	...
	FT_Glyph_Metrics m = mGlypInfos->metrics;
	...
	sprintf(buffer, "%c\nwidth: %d, height: %d\nHorizontal Bearing: %dx%d advance: %d\n\n",
		(char)c, m.width, m.height, m.horiBearingX, m.horiBearingY, m.horiAdvance);
	file.write(buffer, sizeof(char), strlen(buffer));
	// */
}

DRReturn GlyphCalculate::loadGlyph(s32 splitDeepParam)
{	
	// process outline of glyph
	short conturCount = mGlypInfos->outline.n_contours;
	short pointCount = mGlypInfos->outline.n_points;
	if (pointCount == 0) return DR_OK;
	// get max x and y value
	short maxX = 0, maxY = 0;
	short minX = 1000, minY = 1000;
	for (short i = 0; i < pointCount; i++) {
		FT_Vector p = mGlypInfos->outline.points[i];
		if (p.x != (long)((short)p.x) || p.y != (long)((short)p.y)) {
			LOG_ERROR("point value exceed short data range!", DR_ERROR);
		}
		maxX = max(maxX, (short)p.x);
		maxY = max(maxY, (short)p.y);
		minX = min(minX, (short)p.x);
		minY = min(minY, (short)p.y);
	}
	...
	for (short contur = 0; contur < conturCount; contur++) {
		...
		if (contur > 0) start = mGlypInfos->outline.contours[contur - 1] + 1;
		//printf("contur: %d\n", contur);
		for (short i = start; i <= mGlypInfos->outline.contours[contur]; i++)
		{
			FT_Vector p = mGlypInfos->outline.points[i];
			if (i == start) firstPoint = p;
			char f = mGlypInfos->outline.tags[i];
...
DRReturn GlyphCalculate::calculateGrid(FontLoaderTask* parent, Glyph* glyph)
{
	assert(parent != NULL);
