xcopy /Y Y:\UniversumLib\dependencies\lib\Debug\DRCore2.lib  lib\
xcopy /Y Y:\UniversumLib\import\lib\SDL2.lib  lib\
xcopy /Y Y:\UniversumLib\lib\Debug\DRUniversumLib.lib  lib\

xcopy /Y Y:\UniversumLib\dependencies\bin\Debug\DRCore2.dll  bin\
xcopy /Y Y:\UniversumLib\dependencies\bin\Debug\DRImage.dll  bin\
xcopy /Y Y:\UniversumLib\bin\Debug\SDL2.dll  bin\
xcopy /Y Y:\UniversumLib\bin\Debug\DRUniversumLib.dll  bin\

xcopy /Y Y:\UniversumLib\dependencies\Core2\include\* include\Core2\
xcopy /Y Y:\UniversumLib\import\include\sdl\* include\sdl\
xcopy /Y /s Y:\UniversumLib\include\* include\UniversumLib\

xcopy /Y bin\* ..\bin\Debug\



